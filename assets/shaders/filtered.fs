...
extern PRECISION vec2 filtered;
...
float r(vec2 p){
    return cos(filtered.y*cos(p.x*p.y) * 0.2);
}
float rand(vec2 co){
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    //if(abs(0.5 - uv.y) < 0.25){
        vec4 c = colour;
        vec2 p = uv + vec2(seed, 0);
        p *= 40;
        for(int i; i++<16; 
            p = ceil(p + r(ceil(p/10.)) + r(floor(p/5.))*vec2(-1,1)))
            c = sin(p.xyxy + filtered.x);
        if(c.r < 0.05 && c.b < 0.05 && c.g < 0.05){ //&& rand(vec2(floor(filtered.y * 4), seed)) > 0.1){
            return dissolve_mask(vec4(0,0,0,1), texture_coords, uv);
        }
        if(c.g > 0.2 && rand(vec2(floor(filtered.y * 5), seed)) > 0.5){
            colour.g = 1;
            tex.g = min(tex.r + tex.b, 1);
            tex.r = tex.g * 0.6;
            tex.b *= tex.g * 0.6;
        }
    //}

    //colour = vec4(uv.x, uv.y, 1, 1) * filtered.x;
