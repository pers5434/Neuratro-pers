...
vec4 grayscaled(vec4 col)
{
    float lum = 0.2 * col.r + 0.7 * col.g + 0.1* col.b;
    return vec4(vec3(lum), col.a);
}

vec4 argify(vec4 col, Image tex, vec2 tc)
{
    col = grayscaled(col); //grayscale

    col.r = grayscaled(Texel(tex, tc + vec2(0.0015, 0))).r;
    col.b = grayscaled(Texel(tex, tc + vec2(-0.0015, 0))).b;
    return col;
}
...
{
    float maxOffset = 0.1;
    float glitchTime = floor(time * 0.3);
    for (float i = 0.0; i < floor(20 * crt_intensity / 0.16); i += 1.0) {
        float sliceY = random2d(vec2(glitchTime, 20.0 + float(i)));
        float sliceH = sliceY + random2d(vec2(glitchTime, 2000.0 + float(i))) * 0.04;


        float rowY = random2d(vec2(glitchTime, 70.0 + float(i)));
        float rowH = rowY + random2d(vec2(glitchTime, 8000.0 + float(i))) * 0.2;

        float hOffset = randomRange(vec2(glitchTime, 9625.0 + float(i)), -maxOffset, maxOffset);
        if (insideRange(tc.y, sliceY, sliceH) == 1.0 && insideRange(tc.x, rowY, rowH) == 1.0){
        	tc += hOffset;
        }
    }
    ...
    vec3 color_bands = floor(crt_tex.rgb * 16) / 16;
    crt_tex = vec4(min(color_bands, 1.0), crt_tex.a);

    crt_tex = grayscaled(crt_tex);
    ...
    if(crt_amout_adjusted > 0.0000001) {
        crt_tex.r = grayscaled(crt_tex.r*(1.-crt_amout_adjusted) + crt_amout_adjusted*Texel( tex, tc + vec2(0.001*(1. +10.*(artifact_amplifier - 1.))*1600./love_ScreenSize.x, 0.))).r;
        crt_tex.b = grayscaled(crt_tex.b*(1.-crt_amout_adjusted) + crt_amout_adjusted*Texel( tex, tc + vec2(-0.001*(1. +10.*(artifact_amplifier - 1.))*1600./love_ScreenSize.x, 0.))).b;
    }
    ...
    //final_col = argify(final_col, tex, tc);
    ...
    //arg style exaggerated darkness
    
    if(final_col.r < crt_intensity * 0.99 / 0.16){
